...
public function triggerAfterRefundProcess($refundInfo, $orderData, $refund)
{
    ...
    if (Arr::get($refundInfo, 'cancelSubscription') === 'true') {
        if (has_action('fluent_cart/payments/' . $this->slug . '_process_subscription_cancel')) {
            do_action('fluent_cart/payments/' . $this->slug . '_process_subscription_cancel', $refundInfo, $orderData);
        }
    }
...
public static function handle(\FluentCart\App\Events\Order\OrderRefunded $event)
{
    ...
    do_action('fluent_cart/send_email_notification', $event->hook, $event->toArray());
}
```
