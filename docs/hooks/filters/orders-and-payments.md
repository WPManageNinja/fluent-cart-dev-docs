...
<details>
<summary><code>fluent_cart/tax/reverse_charge_line_adjustment</code> &mdash; Filter the per-line reverse-charge price adjustment</summary>

**When it runs:**
Applied while computing the EU B2B reverse-charge adjustment for an inclusive-tax line item, before the line's unit price is reduced to net. The value is the amount (in cents) to subtract from the line subtotal. Core re-clamps the returned value to `0..subtotal` afterwards, so a misbehaving filter cannot produce a negative price.

**Parameters:**
- `$adjustment` (int): The proposed adjustment in cents
- `$context` (array): Line context
    ```php
    $context = [
        'line'    => $line,    // (array) The line item being adjusted
        'rc_mode' => $rcMode,  // (string) The active reverse-charge mode
    ];
    ```

**Returns:** `int` — The adjustment in cents (re-clamped to `0..subtotal` by core)

**Source:** `app/Modules/Tax/TaxModule.php:1349`

**Usage:**
```php
add_filter('fluent_cart/tax/reverse_charge_line_adjustment', function ($adjustment, $context) {
    // Inspect or override the per-line reverse-charge deduction
    return $adjustment;
}, 10, 2);
```
</details>
...
<details>
<summary><code>fluent_cart/tax/validate_eu_vat_number</code> &mdash; Provide a custom EU VAT number validation result</summary>

**When it runs:**
Applied before FluentCart's built-in VIES SOAP validation runs. Return `null` (the default) to let core perform the standard VIES lookup, or short-circuit it by returning a `WP_Error` (validation failed) or an array (validation result), in which case core uses your result and skips VIES. Use this to plug in a third-party VAT validation service or a cache.

**Parameters:**
- `$result` (null|array|\WP_Error): The pre-resolved validation result (default `null`)
- `$context` (array): VAT context
    ```php
    $context = [
        'country_code' => 'DE',          // (string) The EU country code
        'vat_number'   => '123456789',   // (string) The VAT number to validate
    ];
    ```

**Returns:** `null` to fall through to VIES, or an `array`/`WP_Error` result to short-circuit

**Source:** `app/Modules/Tax/TaxModule.php:2090`

**Usage:**
```php
add_filter('fluent_cart/tax/validate_eu_vat_number', function ($result, $context) {
    // Use a cached result if available
    $cached = get_transient('vat_' . $context['country_code'] . $context['vat_number']);
...
<details>
<summary><code>fluent_cart/pdf/generate_receipt</code> &mdash; Generate (or override) the receipt PDF file path</summary>

**When it runs:**
Applied when a receipt PDF is requested — both when downloading a receipt and when attaching a receipt PDF to an email. Core passes `null` as the default; the Pro PDF add-on hooks in to generate the file and return its path. Return a path to a generated PDF, or keep `null` to indicate no PDF.

**Parameters:**
- `$pdfPath` (string|null): The generated PDF file path (default `null`)
- `$context` (array): Receipt context
    ```php
    $context = [
        'order'       => $order,            // (Order) The order to render
        'template_id' => 'order_receipt',   // (string) The PDF template ID
    ];
    ```

**Returns:** `string|null` — Absolute path to the generated PDF, or `null`

**Source:** `app/Hooks/Handlers/ShortCodes/ReceiptHandler.php:98` and `app/Services/Email/EmailNotificationMailer.php:358`

**Usage:**
```php
add_filter('fluent_cart/pdf/generate_receipt', function ($pdfPath, $context) {
    if ($pdfPath) {
        return $pdfPath; // already generated by the PDF add-on
    }
    // Generate your own PDF and return its path
    return $pdfPath;
}, 20, 2);
```
</details>
...
<details>
<summary><code>fluent_cart/paddle/webhook_auto_recover_enabled</code> <Badge type="warning" text="Pro" /> &mdash; Toggle API-pull recovery for Paddle webhooks with a failed signature</summary>

**When it runs:**
Applied during Paddle webhook (IPN) processing when signature verification fails. When the filter returns `true` (the default), the handler attempts to recover by re-pulling the entity from the Paddle API instead of rejecting the request; when it returns `false`, the webhook is rejected with a `401 Invalid webhook signature`. This filter replaces the former `auto_recover_unverified_webhooks` setting.

**Parameters:**
- `$isAutoRecoverEnabled` (bool): Whether auto-recovery is enabled (default `true`)
- `$context` (array): Webhook context
    ```php
    $context = [
        'payload' => $payload,   // (array) The parsed webhook payload
    ];
    ```

**Returns:** `bool` — `true` to attempt API-pull recovery, `false` to reject the unverified webhook

**Source:** `fluent-cart-pro/app/Modules/PaymentMethods/PaddleGateway/Webhook/IPN.php:75`

**Usage:**
```php
add_filter('fluent_cart/paddle/webhook_auto_recover_enabled', function ($enabled, $context) {
    // Reject all unverified webhooks instead of recovering via API pull
    return false;
}, 10, 2);
```
</details>
